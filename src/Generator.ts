...
export default class Generator {
  ...
  async fetchApi<T = any>(url: string, query: Record<string, any>, options?: Record<string, any>): Promise<T> {
  ...
  fetchExport = memoize(
    ...
    ({ serverUrl, token, promiseKey }: SyntheticalConfig) => `${serverUrl}|${token}|${promiseKey}`,
  )
